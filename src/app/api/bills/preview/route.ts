...
export async function GET(request: NextRequest) {
  try {
    ...
    const customersWithBillsSet = new Set(
      existingBills.map((b) => b.customerId),
    );
    ...
    // Separate by status
    const billsByStatus = existingBills.reduce(
      (acc, bill) => {
    ...
    const categoryAnalysis = customersWithReadingsNoBills.reduce(
      (acc, customer) => {
      ...
      {} as Record<
        string,
        { count: number; totalUnits: number; hasTariff: boolean }
      >,
    );
    ...
    Object.keys(categoryAnalysis).forEach((category) => {
      categoryAnalysis[category].hasTariff = tariffCategories.has(
        category as any,
      );
    });
    ...
    const totalEligibleForGeneration =
      totalActiveCustomers.count - customersWithBillsSet.size;
    ...
    for (const customer of customersWithReadingsNoBills) {
      ...
      if (tariff) {
        ...
        const electricityDuty =
          baseAmount * (Number(tariff.electricityDutyPercent ?? 0) / 100);
        const gstAmount =
          (baseAmount + fixedCharges + electricityDuty) *
          (Number(tariff.gstPercent ?? 0) / 100);
        const totalAmount =
    ...
    const eligibleWithoutReadings =
      totalEligibleForGeneration - customersWithReadingsNoBills.length;
    ...
    const preview = {
      ...
      existingBills: {
        ...
        statusCounts: {
          ...
          issued:
            (billsByStatus["issued"]?.length || 0) +
            (billsByStatus["generated"]?.length || 0),
