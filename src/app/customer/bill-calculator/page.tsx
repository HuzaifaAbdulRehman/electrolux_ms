...
export default function BillCalculator() {
  ...
  const calculateCharges = (units: number, tariff: any) => {
    ...
    if (tariff.slabs) {
      for (const slab of tariff.slabs) {
        ...
        const unitsInSlab = Math.max(
          ...
          Math.min(
            units - consumedUnits,
            slabEnd - Math.max(slabStart, consumedUnits),
          ),
        );
    ...
    const electricityDuty = Math.round(
      energyCharge * (tariff.electricityDutyPercent / 100),
    );

    // Apply GST on (baseAmount + fixedCharges + electricityDuty)
    const gst = Math.round(
      (energyCharge + fixedCharge + electricityDuty) *
        (tariff.gstPercent / 100),
    );

    const subtotal = Math.round(energyCharge + fixedCharge);
    const totalAmount = Math.round(
      energyCharge + fixedCharge + electricityDuty + gst,
    );
    ...
    return (
      ...
      >
        <div className="h-full flex items-center justify-center">
          <div className="text-center">
            <p className="text-gray-600 dark:text-gray-400">
              No tariff data available for selected connection type
            </p>
          </div>
        </div>
      </DashboardLayout>
                  ...
                  <div className="space-y-4">
                    ...
                    <div className="space-y-3">
                      ...
                      <div className="flex justify-between items-center pb-3 border-b border-gray-200 dark:border-white/10">
                        <span className="text-gray-600 dark:text-gray-400">
                          Electricity Duty (
                          {selectedTariff.electricityDutyPercent}%)
                        </span>
                        <span className="text-gray-900 dark:text-white font-semibold">
                          Rs. {electricityDuty}
                        </span>
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
              <div className="bg-gradient-to-r from-blue-500/10 to-cyan-500/10 backdrop-blur-xl rounded-xl p-4 border border-blue-500/20">
                ...
                <p className="text-xs text-gray-600 dark:text-gray-400">
                  Bills calculated using slab-based tariff rates encouraging
                  conservation.
                </p>
              </div>
