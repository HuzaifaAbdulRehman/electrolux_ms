  ...
  try {
    ...
    if (session.user.userType !== "employee") {
      ...
      return NextResponse.json(
        { error: "Forbidden - Only employees can generate bills" },
        { status: 403 },
      );
    }
    ...
    console.log("[Bills Generate POST] Request data:", {
      customerId,
      billingMonth,
    });
    ...
    if (customer.status === "suspended") {
      console.log(
        "[Bills Generate POST] ⚠️  WARNING: Generating bill for suspended customer:",
        customerId,
      );
    }
    ...
    if (!latestReading) {
      return NextResponse.json(
        {
          error:
            "No meter reading found for this billing month. Please record meter reading first.",
        },
        { status: 400 },
      );
    }
    ...
    if (!activeTariff) {
      console.error(
        "[Bill Generation] No active tariff found for category:",
        customerCategory,
      );
      return NextResponse.json(
    ...
    if (unitsConsumed === 0) {
      console.log(
        "[Bill Generation] WARNING: Zero consumption detected for customer",
        customerId,
      );
    }
    let baseAmount = 0;
    ...
    for (let i = 0; i < slabs.length; i++) {
      ...
      if (end !== null && end < start) {
        throw new Error(
          `Slab ${i + 1} has end (${end}) less than start (${start})`,
        );
      }
      ...
      if (i < slabs.length - 1) {
        ...
        if (end !== null && end >= nextStart) {
          throw new Error(
            `Slab ${i + 1} overlaps with slab ${i + 2}: end=${end}, nextStart=${nextStart}`,
          );
        }
      }
    }
    ...
    const electricityDuty = Math.round(
      baseAmount * (Number(activeTariff.electricityDutyPercent ?? 0) / 100),
    );
    const gstAmount = Math.round(
      (baseAmount + fixedCharges + electricityDuty) *
        (Number(activeTariff.gstPercent ?? 0) / 100),
    );
    ...
    while (attempts < maxAttempts) {
      billNumber = `BILL-${new Date().getFullYear()}-${Math.floor(
    ...
    try {
      const result = await db.transaction(async (tx) => {
        ...
        if (existingBill) {
          console.log(
            "[Bills Generate POST] ❌ Bill already exists (detected in transaction)",
          );
        ...
        console.log(
          "[Bills Generate POST] ✅ Bill inserted successfully - ID:",
          newBillId,
        );

        // Update customer's outstanding balance and last bill amount
        const newOutstandingBalance =
          parseFloat(customer.outstandingBalance || "0") + totalAmount;
        await tx
          .update(customers)
          .set({
        ...
        console.log(
          "[Bills Generate POST] ✅ Bill object constructed with ID:",
          newBillId,
        );
    ...
    if (!createdBill) {
      console.error(
        "[Bills Generate POST] ❌ Bill object is null/undefined after transaction!",
      );
      return NextResponse.json(
        {
    ...
    try {
      ...
      console.warn(
        "[Bills Generate POST] ⚠️ Failed to update bill request status (non-critical):",
        updateError,
      );
    }
    ...
    try {
      ...
      console.warn(
        "[Bills Generate POST] ⚠️ Failed to create notification (non-critical):",
        notifError,
      );
    }
    ...
    console.error(
      "[Bills Generate POST] Full error object:",
