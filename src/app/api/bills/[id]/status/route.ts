...
export async function PATCH(
  ...
  try {
    ...
    console.log("[Bill Status Update] Request:", {
      billId,
      status,
      paymentMethod,
      transactionRef,
    ...
    if (status === "paid") {
      try {
        ...
        const receiptNumber = `RCPT-${new Date().getFullYear()}-${Math.floor(
        ...
        const insertResult = await db.insert(payments).values(paymentData);
        ...
        console.log(
          "[Payment Creation] ✅ Payment record created:",
          paymentRecord?.id,
        );
        ...
        try {
          ...
          console.error(
            "[Payment Creation] ⚠️ Notification failed:",
            notifError,
          );
        ...
        console.error(
          "[Payment Creation] ❌ Failed to create payment record:",
          paymentError,
        );
        return NextResponse.json(
