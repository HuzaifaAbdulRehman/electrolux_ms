...
export default function ViewBills() {
  ...
  const fetchBills = async () => {
    try {
      ...
      const transformedBills = result.data.map((bill: any) => {
        ...
        const tariffSlabs = calculateTariffSlabs(
          unitsConsumed,
          bill.tariffSlabs,
        );
        ...
        return {
          ...
          month: new Date(bill.billingMonth).toLocaleDateString("en-US", {
            month: "long",
            year: "numeric",
          }),
  ...
  const billsWithConsumptionOnly = bills.filter(
    (b) => safeNumber(b.units, 0) > 0,
  );
  ...
  const totalPaid = billsWithConsumptionOnly
    .filter((b) => b.status === "paid")
    .reduce((sum, b) => sum + safeNumber(b.amount, 0), 0);
  const avgConsumption =
    billsWithConsumptionOnly.length > 0
      ? Math.round(
          billsWithConsumptionOnly.reduce(
            (sum, b) => sum + safeNumber(b.units, 0),
            0,
          ) / billsWithConsumptionOnly.length,
        )
      : 0;
  const avgAmount =
    billsWithConsumptionOnly.length > 0
      ? billsWithConsumptionOnly.reduce(
          (sum, b) => sum + safeNumber(b.amount, 0),
          0,
        ) / billsWithConsumptionOnly.length
      : 0;
  ...
  const consumptionChange =
    currentMonth && lastMonth && safeNumber(lastMonth.units, 0) > 0
      ? (
          ((safeNumber(currentMonth.units, 0) -
            safeNumber(lastMonth.units, 0)) /
            safeNumber(lastMonth.units, 1)) *
          100
        ).toFixed(1)
      : "0";
  const amountChange =
    currentMonth && lastMonth && safeNumber(lastMonth.amount, 0) > 0
      ? (
          ((safeNumber(currentMonth.amount, 0) -
            safeNumber(lastMonth.amount, 0)) /
            safeNumber(lastMonth.amount, 1)) *
          100
        ).toFixed(1)
      : "0";
  ...
  const combinedTrendData = {
    labels: billsWithConsumptionOnly
      .map((b) => b.month.split(" ")[0])
      .reverse(),
    datasets: [
      {
        label: "Units Consumed (kWh)",
        data: billsWithConsumptionOnly
          .map((b) => safeNumber(b.units, 0))
          .reverse(),
        ...
        <div className="max-w-[1920px] mx-auto space-y-6 pb-8">
          <div className="bg-white dark:bg-white/5 backdrop-blur-xl rounded-2xl p-10 border border-gray-200 dark:border-white/10 text-center">
            ...
            <p className="text-gray-600 dark:text-gray-400 mb-4">
              Your first bill will appear after your first meter reading.
            </p>
            <button
      ...
      <div className="max-w-[1920px] mx-auto space-y-6 pb-8">
          ...
          <div className="flex items-center justify-between">
            <div>
              ...
              <p className="text-gray-600 dark:text-gray-400">
                View detailed bill history, analytics, and request new bills
              </p>
            </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-4 gap-6">
          ...
          <div className="bg-gradient-to-br from-orange-500/20 to-red-500/20 backdrop-blur-xl rounded-2xl p-6 border border-orange-500/30">
            ...
            <p className="text-3xl font-bold text-gray-900 dark:text-white">
              {currentMonth ? formatUnits(currentMonth.units) : "N/A"}
            </p>
            <p
              className={`text-sm mt-1 ${parseFloat(consumptionChange) > 0 ? "text-red-400" : "text-green-400"}`}
            >
              {parseFloat(consumptionChange) > 0 ? "+" : ""}
              {consumptionChange}% vs last month
            </p>
          </div>
        </div>
          ...
          <div className="flex items-center justify-between mb-6">
            <div>
              <h2 className="text-xl font-bold text-gray-900 dark:text-white">
                Consumption & Cost Trend Analysis
              </h2>
              <p className="text-sm text-gray-600 dark:text-gray-400 mt-1">
                6-month historical comparison of energy usage and billing
                amounts
              </p>
            </div>
            <div className="flex items-center space-x-4">
          ...
          <div className="mt-4 grid grid-cols-3 gap-4">
            ...
            <div className="bg-gradient-to-br from-purple-500/10 to-pink-500/10 rounded-xl p-4 border border-purple-500/20">
              ...
              <p className="text-2xl font-bold text-purple-400">
                {formatCurrency(
                  avgConsumption > 0 ? avgAmount / avgConsumption : 0,
                  "Rs.",
                )}
              </p>
            </div>
                    ...
                    >
                      ...
                      <td className="px-6 py-4">
                        ...
                        <p className="text-xs text-gray-600 dark:text-gray-400">
                          Issued: {safeDate(bill.issueDate)}
                        </p>
                      </td>
                      <td className="px-6 py-4">
                        <p className="text-sm font-semibold text-gray-900 dark:text-white">
                          {formatUnits(bill.units)}
                        </p>
                      </td>
                      <td className="px-6 py-4">
                        <p className="text-sm font-bold text-yellow-400">
                          {formatCurrency(bill.amount, "Rs.")}
                        </p>
                      </td>
                      <td className="px-6 py-4">
                      ...
                      <td className="px-6 py-4">
                        <span
                          className={`inline-flex items-center space-x-1 px-3 py-1 rounded-full text-xs font-semibold border ${getStatusColor(bill.status)}`}
                        >
              ...
              <div className="p-6 space-y-6">
                ...
                <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
                  ...
                  <div className="bg-gray-50 dark:bg-white/5 rounded-xl p-4 border border-gray-200 dark:border-white/10">
                    ...
                    <p className="text-gray-900 dark:text-white font-semibold">
                      {formatUnits(selectedBill.units)}
                    </p>
                  </div>
                    ...
                    {selectedBill.breakdown.tariffSlabs.map((slab, index) => (
                      ...
                      >
                        <div className="flex items-center space-x-3">
                          ...
                          <div>
                            <p className="text-gray-900 dark:text-white font-semibold">
                              {formatUnits(slab.units)}
                            </p>
                            <p className="text-gray-600 dark:text-gray-400 text-xs">
                              @ {formatCurrency(slab.rate, "Rs.")}/kWh
                            </p>
                            {(slab as TariffSlab).range && (
                              <p className="text-gray-500 dark:text-gray-500 text-xs mt-1">
                                Range: {(slab as TariffSlab).range}
                              </p>
                            )}
                          </div>
                        </div>
                ...
                <div className="bg-gray-50 dark:bg-white/5 rounded-xl p-5 border border-gray-200 dark:border-white/10">
                  ...
                  <div className="space-y-3">
                    <div className="flex items-center justify-between py-2 border-b border-gray-200 dark:border-white/10">
                      <p className="text-gray-600 dark:text-gray-400">
                        Base Amount (Energy Charges)
                      </p>
                      <p className="text-gray-900 dark:text-white font-semibold">
                        {formatCurrency(
                          selectedBill.breakdown.baseAmount,
                          "Rs.",
                        )}
                      </p>
                    </div>
                    <div className="flex items-center justify-between py-2 border-b border-gray-200 dark:border-white/10">
                      ...
                      <p className="text-gray-900 dark:text-white font-semibold">
                        {formatCurrency(
                          selectedBill.breakdown.fixedCharges,
                          "Rs.",
                        )}
                      </p>
                    </div>
                    <div className="flex items-center justify-between py-2 border-b border-gray-200 dark:border-white/10">
                      ...
                      <p className="text-gray-900 dark:text-white font-semibold">
                        {formatCurrency(
                          selectedBill.breakdown.electricityDuty,
                          "Rs.",
                        )}
                      </p>
                    </div>
                    <div className="flex items-center justify-between py-2 border-b border-gray-200 dark:border-white/10">
                      ...
                      <p className="text-gray-900 dark:text-white font-semibold">
                        {formatCurrency(selectedBill.breakdown.gst, "Rs.")}
                      </p>
                    </div>
                    <div className="flex items-center justify-between py-3 pt-4 border-t-2 border-gray-300 dark:border-white/20">
                      ...
                      <p className="text-yellow-400 font-bold text-2xl">
                        {formatCurrency(
                          selectedBill.breakdown.totalAmount,
                          "Rs.",
                        )}
                      </p>
                    </div>
                      ...
                      <div>
                        ...
                        <p className="text-white font-semibold">
                          {selectedBill.paidDate
                            ? safeDate(selectedBill.paidDate)
                            : "N/A"}
                        </p>
                      </div>
                      <div>
                ...
                <div className="flex items-center space-x-3">
                  <button
                    onClick={() =>
                      selectedBill && handleDownloadPDF(selectedBill)
                    }
