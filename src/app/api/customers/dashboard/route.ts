...
export async function GET(request: NextRequest) {
  try {
    ...
    console.log(
      "[Dashboard API] Fetching dashboard data for:",
    ...
    const activeCustomers =
      customersByStatus.find((c) => c.status === "active")?.count || 0;
    const suspendedCustomers =
      customersByStatus.find((c) => c.status === "suspended")?.count || 0;
    const inactiveCustomers =
      customersByStatus.find((c) => c.status === "inactive")?.count || 0;
    const totalCustomersAll =
      activeCustomers + suspendedCustomers + inactiveCustomers;
    ...
    const monthsWithBillsResult = await db
      .select({
        count: sql<number>`COUNT(DISTINCT DATE_FORMAT(${bills.billingMonth}, '%Y-%m'))`,
      })
      .from(bills)
      .where(sql`${bills.billingMonth} >= ${startDateStr}`);
    ...
    const actualMonths =
      (monthsWithBillsResult[0]?.count || 0) > 0
        ? monthsWithBillsResult[0].count
        : 1;
    ...
    if (period !== "all") {
      ...
      const prevActualMonths =
        (prevMonthsWithBillsResult[0]?.count || 0) > 0
          ? prevMonthsWithBillsResult[0].count
          : 1;
      ...
      previousMetrics = {
        ...
        collectionRate:
          prevTotalBills.count > 0
            ? parseFloat(
                ((prevPaidBills.count / prevTotalBills.count) * 100).toFixed(1),
              )
            : 0,
        totalBills: prevTotalBills.count,
        averageBillAmount:
          prevTotalBills.count > 0
            ? prevRevenueTotal / prevTotalBills.count
            : 0,
    ...
    const currentCollectionRate =
      totalBills.count > 0
        ? parseFloat(((paidBills.count / totalBills.count) * 100).toFixed(1))
        : 0;
    const currentAvgBillAmount =
      totalBills.count > 0 ? revenueTotal / totalBills.count : 0;
    ...
    const metrics = {
      ...
      paymentRate:
        totalBills.count > 0 ? (paidBills.count / totalBills.count) * 100 : 0,
      ...
      trends: previousMetrics
        ? {
            ...
            collectionRate: calculateTrend(
              currentCollectionRate,
              previousMetrics.collectionRate,
            ),
            totalCustomers: calculateTrend(
              totalCustomersAll,
              previousMetrics.activeCustomers,
            ),
            averageBillAmount: calculateTrend(
              currentAvgBillAmount,
              previousMetrics.averageBillAmount,
            ),
            activeCustomers: calculateTrend(
              activeCustomers,
              previousMetrics.activeCustomers,
            ),
          }
    ...
    const revenueByCategoryFormatted = revenueByCategory.reduce(
      (acc, item) => {
    ...
    const paymentMethodsFormatted = paymentMethods.reduce(
      (acc, item) => {
        acc[item.method] = {
          count: item.count,
          total: item.total || 0,
    ...
    const billsStatusFormatted = billsStatusDistribution.reduce(
      (acc, item) => {
        acc[item.status] = {
          count: item.count,
    ...
    const connectionTypeFormatted = connectionTypeDistribution.reduce(
      (acc, item) => {
    ...
    const dashboardData = {
      ...
      workOrderStats: workOrderStats.reduce(
        (acc, item) => {
      ...
      collectionRateTrend: collectionRateTrend.map((item) => ({
        ...
        collectionRate:
          item.totalBills > 0
            ? parseFloat(((item.paidBills / item.totalBills) * 100).toFixed(1))
            : 0,
