...
export async function POST(request: NextRequest) {
  try {
    ...
    console.log(
      "[Bulk Generation] Starting bulk bill generation for:",
      billingMonth,
    );
    ...
    console.log(
      `[Bulk Generation] Found ${allCustomers.length} active customers`,
    );
    ...
    const tariffsMap = new Map(tariffsList.map((t) => [t.category, t]));
    ...
    const allSlabs = tariffIds.length
      ? await db
          .select()
          .from(tariffSlabs)
          .where(inArray(tariffSlabs.tariffId, tariffIds))
          .orderBy(tariffSlabs.tariffId, tariffSlabs.slabOrder)
      : ([] as any[]);
    ...
    for (let i = 0; i < allCustomers.length; i += BATCH_SIZE) {
      ...
      console.log(
        `[Bulk Generation] Processing batch ${Math.floor(i / BATCH_SIZE) + 1}/${Math.ceil(allCustomers.length / BATCH_SIZE)}`,
      );
      ...
      for (const customer of batch) {
        ...
        try {
          ...
          if (!reading) {
            console.log(
              `[Bulk Generation] No reading for customer ${customer.accountNumber} - generating automatic reading`,
            );
            ...
            try {
              ...
              const prevMonthStart = previousMonthDate
                .toISOString()
                .split("T")[0];
              ...
              if (!lastMonthReading) {
                ...
                if (anyPreviousReading) {
                  ...
                  previousReading = parseFloat(
                    anyPreviousReading.currentReading,
                  );
                  console.log(
                    `[Bulk Generation] Using any available reading: ${previousReading} for ${customer.accountNumber}`,
                  );
                } else {
                  ...
                  const connectionDate = new Date(
                    customer.connectionDate || billingMonth,
                  );
                  ...
                  if (connectionDate >= billingMonthDate) {
                    ...
                    previousReading = 0;
                    console.log(
                      `[Bulk Generation] NEW customer connected this month: ${customer.accountNumber} - Starting from 0`,
                    );
                  } else {
                    ...
                    const monthsSinceConnection = Math.floor(
                      (billingMonthDate.getTime() - connectionDate.getTime()) /
                        (1000 * 60 * 60 * 24 * 30),
                    );
                    ...
                    const monthlyDefault =
                      defaults[customer.connectionType || "Residential"] || 200;
                    previousReading =
                      monthlyDefault * Math.max(1, monthsSinceConnection);

                    console.log(
                      `[Bulk Generation] NEW customer (${monthsSinceConnection} months old): ${customer.accountNumber} - Estimated previous: ${previousReading}`,
                    );
                  }
                }
              } else {
              ...
              if (historicalReadings.length >= 6) {
                const totalConsumption = historicalReadings.reduce(
                  (sum, r) => sum + parseFloat(r.unitsConsumed || "0"),
                  0,
                );
                averageConsumption =
                  totalConsumption / historicalReadings.length;
                console.log(
                  `[Bulk Generation] Using 6-month average: ${averageConsumption.toFixed(2)} kWh`,
                );
                ...
                averageConsumption =
                  totalConsumption / historicalReadings.length;
                console.log(
                  `[Bulk Generation] Using ${historicalReadings.length}-month average: ${averageConsumption.toFixed(2)} kWh`,
                );
              } else {
                ...
                if (categoryPeers.length > 0) {
                  const totalConsumption = categoryPeers.reduce(
                    (sum, r) => sum + parseFloat(r.unitsConsumed || "0"),
                    0,
                  );
                  averageConsumption = totalConsumption / categoryPeers.length;
                  console.log(
                    `[Bulk Generation] Using category average: ${averageConsumption.toFixed(2)} kWh`,
                  );
                } else {
                  ...
                  if (zonePeers.length > 0) {
                    const totalConsumption = zonePeers.reduce(
                      (sum, r) => sum + parseFloat(r.unitsConsumed || "0"),
                      0,
                    );
                    averageConsumption = totalConsumption / zonePeers.length;
                    console.log(
                      `[Bulk Generation] Using zone average: ${averageConsumption.toFixed(2)} kWh`,
                    );
                  } else {
                    ...
                    averageConsumption =
                      defaults[customer.connectionType || "Residential"] || 200;
                    console.log(
                      `[Bulk Generation] Using default consumption: ${averageConsumption} kWh`,
                    );
                  }
                }
              }
              ...
              const variance = isFirstReading
                ? 1.0
                : 0.85 + Math.random() * 0.3; // No variance for brand new customers
              const newConsumption = Math.round(averageConsumption * variance);

              // Validate consumption is reasonable
              if (newConsumption < 0) {
                console.warn(
                  `[Bulk Generation] Negative consumption calculated for ${customer.accountNumber}, using 0`,
                );
              ...
              console.log(
                `[Bulk Generation] ${isFirstReading ? "FIRST" : "Auto"} reading for ${customer.accountNumber}: prev=${previousReading}, consumed=${newConsumption}, new=${newReading}`,
              );
              ...
              const readingInsertResult = await db
              ...
              console.log(
                `[Bulk Generation] Created ${isFirstReading ? "FIRST" : "automatic"} reading for ${customer.accountNumber}: ${newConsumption} kWh (variance: ${(variance * 100).toFixed(1)}%)`,
              );
              ...
              console.error(
                `[Bulk Generation] Error generating automatic reading for ${customer.accountNumber}:`,
                autoReadingError,
              );
          ...
          const billCalculation = calculateBillCharges(
            unitsConsumed,
            tariff,
            tariffSlabRows,
          );
          ...
          if (customer.userId) {
            db.insert(notifications)
              ...
              .catch((err) => {
                console.error(
                  `[Bulk Generation] Failed to send notification to customer ${customer.accountNumber}:`,
                  err,
                );
              });
          }
          ...
          console.log(
            `[Bulk Generation] Generated bill for ${customer.accountNumber}: Rs ${billCalculation.totalAmount.toFixed(2)}`,
          );
        } catch (error: any) {
          console.error(
            `[Bulk Generation] Error processing customer ${customer.accountNumber}:`,
            error,
          );
          stats.failed.push({
            customerId: customer.id,
    ...
    return NextResponse.json({
      ...
      summary: {
        ...
        duration: `${(duration / 1000).toFixed(2)}s`,
        averageTimePerBill:
          stats.billsGenerated > 0
            ? `${(duration / stats.billsGenerated).toFixed(2)}ms`
            : "N/A",
      },
...
function calculateBillCharges(
  ...
  const electricityDuty = Math.round(
    baseAmount * (parseFloat(tariff.electricityDutyPercent) / 100),
  );
  const gstAmount = Math.round(
    (baseAmount + fixedCharges + electricityDuty) *
      (parseFloat(tariff.gstPercent) / 100),
  );
  const totalAmount = Math.round(
    baseAmount + fixedCharges + electricityDuty + gstAmount,
  );
