...
export async function PATCH(
  ...
  try {
    ...
    console.log(
      "[TARIFF UPDATE] Received request body:",
    ...
    if (!category || !fixedCharge || !effectiveDate) {
      return NextResponse.json(
        {
          error:
            "Missing required fields: category, fixedCharge, effectiveDate",
        },
        { status: 400 },
      );
    }
    ...
    if (!slabs || !Array.isArray(slabs) || slabs.length < 5) {
      console.log("[TARIFF UPDATE] Slabs validation failed:", {
        slabs,
        isArray: Array.isArray(slabs),
        length: slabs?.length,
      });
      return NextResponse.json(
        {
    ...
    const formattedEffectiveDate = new Date(effectiveDate)
      .toISOString()
      .split("T")[0];
    ...
    const newTariffData: any = {
      ...
      timeOfUsePeakRate: timeOfUsePeakRate
        ? timeOfUsePeakRate.toString()
        : "0.00",
      timeOfUseNormalRate: timeOfUseNormalRate
        ? timeOfUseNormalRate.toString()
        : "0.00",
      timeOfUseOffpeakRate: timeOfUseOffpeakRate
        ? timeOfUseOffpeakRate.toString()
        : "0.00",
    };

    console.log(
      "[TARIFF UPDATE] Creating new tariff version:",
      JSON.stringify(newTariffData, null, 2),
    );
    ...
    await db
      ...
      .where(and(eq(tariffs.id, tariffId), sql`${tariffs.validUntil} IS NULL`));
    ...
    const [newTariff] = await db
      ...
      .from(tariffs)
      .where(
        and(eq(tariffs.category, category), sql`${tariffs.validUntil} IS NULL`),
      )
