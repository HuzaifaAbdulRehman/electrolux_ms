    ...
    try {
      ...
      if (result.success) {
        ...
        if (result.stats) {
          ...
          const issued = result.data.filter(
            (b: any) => b.status === "issued",
          ).length;
          const paid = result.data.filter(
            (b: any) => b.status === "paid",
          ).length;
  ...
  const handleStatusUpdate = async (
    ...
    try {
      ...
      if (result.success) {
        ...
        alert(
          "Failed to update bill status: " + (result.error || "Unknown error"),
        );
      }
    } catch (err) {
  ...
  const handleMarkAsPaid = async () => {
    ...
    try {
      ...
      if (checkResult.success && checkResult.data.status === "paid") {
        alert(
          "⚠️ This bill was just paid by the customer!\n\nPayment already recorded. Refreshing...",
        );
        setShowPaymentModal(false);
    ...
    const confirmMessage =
      `Confirm marking Bill #${selectedBill.id} as PAID?\n\n` +
                  ...
                  {bills.map((bill) => (
                    ...
                    >
                      ...
                      <td className="px-6 py-4 text-sm text-gray-600 dark:text-gray-400">
                        {new Date(bill.billingPeriod).toLocaleDateString(
                          "en-US",
                      ...
                      <td className="px-6 py-4 text-sm font-semibold text-gray-900 dark:text-white">
                        Rs {parseFloat(bill.totalAmount).toLocaleString()}
                      </td>
                      <td className="px-6 py-4">
                        <span
                          className={`inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium border ${getStatusColor(bill.status)}`}
                        >
          ...
          >
            ...
            >
              ...
              <div className="flex-1 overflow-y-auto p-6 space-y-6">
                ...
                <div className="flex items-center justify-between">
                  <span
                    className={`inline-flex items-center px-4 py-2 rounded-full text-sm font-semibold border ${getStatusColor(selectedBill.status)}`}
                  >
                  ...
                  <div className="space-y-3">
                    ...
                    <div className="flex justify-between items-center">
                      ...
                      <span className="font-semibold text-gray-900 dark:text-white">
                        Rs{" "}
                        {parseFloat(
                          selectedBill.baseAmount || 0,
                        ).toLocaleString()}
                      </span>
                    </div>
                    {selectedBill.additionalCharges &&
                      parseFloat(selectedBill.additionalCharges) > 0 && (
                        <div className="flex justify-between items-center">
                          <span className="text-gray-600 dark:text-gray-400">
                            Additional Charges
                          </span>
                ...
                <div className="bg-gray-50 dark:bg-white/5 rounded-xl p-4 border border-gray-200 dark:border-white/10">
                  ...
                  <div className="grid grid-cols-2 gap-4">
                    <div>
                      ...
                      <p className="text-base font-semibold text-gray-900 dark:text-white">
                        {new Date(selectedBill.issueDate).toLocaleDateString(
                          "en-US",
                          { month: "short", day: "numeric", year: "numeric" },
                        )}
                      </p>
                ...
                <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center space-y-3 sm:space-y-0">
                  ...
                  <div className="flex flex-wrap gap-2">
                    ...
                    {selectedBill.status === "pending" && (
                      <button
                        onClick={() =>
                          handleStatusUpdate(selectedBill.id, "issued")
                        }
                    ...
                    {(selectedBill.status === "issued" ||
                      ...
                      <button
                        onClick={() => {
                          alert(
                            "Payment reminder will be sent to customer via email/SMS (future feature)",
                          );
                        }}
                ...
                <div className="flex items-center justify-between">
                  <div>
                    ...
                    <p className="text-green-100 text-sm mt-1">
                      Bill #{selectedBill.id} - Rs{" "}
                      {parseFloat(selectedBill.totalAmount).toLocaleString()}
                    </p>
                  </div>
                  <button
                    onClick={() => setShowPaymentModal(false)}
              ...
              <div className="p-6 space-y-4">
                ...
                <div className="bg-yellow-50 dark:bg-yellow-500/10 border border-yellow-200 dark:border-yellow-500/30 rounded-lg p-3">
                  <div className="flex items-start space-x-2">
                    ...
                    <div className="text-sm text-yellow-800 dark:text-yellow-200">
                      ...
                      <p>
                        Ensure payment is received and verified before marking
                        as paid. This action creates an audit trail.
                      </p>
                    </div>
                  </div>
                </div>
                ...
                <div>
                  ...
                  <p className="text-xs text-gray-500 dark:text-gray-400 mt-1">
                    Minimum 5 characters. Use bank transaction ID, receipt
                    number, or cheque number.
                  </p>
                </div>
